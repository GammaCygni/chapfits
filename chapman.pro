...
;----------------------------------------------------------------
;this function, N, is the sum of two Chapman layers, used to fit 
;to MGS electron density profiles with IDL's CURVEFIT procedure
;in order to deterime the best values of A.
;
;INPUT
; z - an altitude scale
...
N2=A[0]*exp(.5*((1.0-(z-A[1])/A[2])-exp(-(z-A[1])/A[2])))

N1=A[3]*exp(.5*((1.0-(z-A[4])/A[5])-exp(-(z-A[4])/A[5])))
